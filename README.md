...
공개 패키지 표면은 표준 ESM runtime과 TypeScript 소비자를 위한 compiled package다. root export는 `dist/index.js`와 `dist/index.d.ts`이고, 하위 export는 `zdp-client-sdks/typed-fetch`, `zdp-client-sdks/typed-fetch/api-operations`만 허용한다. `files` whitelist는 `dist/`, `contracts/`, 운영 문서, `LICENSE`만 포함하며 `src/`, checker, generation-plan 구현, tests는 npm tarball에 넣지 않는다. build 단계는 emitted JavaScript와 declaration의 상대 import를 Node ESM이 직접 해석할 수 있는 명시적 `.js` 경로로 정규화한다. live base URL, refresh token storage, provider secret, generated language-specific SDK artifact도 패키지에 포함하지 않는다.

공개 릴리스는 `package.json` 버전과 같은 `v<version>` tag가 `main`에 포함된 커밋을 가리킬 때만 실행된다. GitHub Actions의 npm Trusted Publisher가 OIDC로 정확한 tarball을 공개하며, 장기 npm token이나 로컬 `npm publish`는 사용하지 않는다. 같은 tarball은 publish 전에 빈 소비자에서 설치되고 Node와 Bun direct import 및 TypeScript `NodeNext` declaration resolution을 통과해야 한다. 일반 CI는 Vite current major build도 확인한다. publish 뒤에는 npm `gitHead`, integrity, registry signature, SLSA provenance, GitHub Release 자산까지 같은 커밋을 가리키는지 확인한다.
