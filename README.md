...
- schema별 request/response 모델 생성 결과
...
- SDK는 API export plan handoff에서 검증한 operation metadata를 TypeScript typed fetch runtime에 연결 가능한 operation definitions로 노출한다. 이 단계는 schema별 encoder/decoder를 생성하지 않으므로 request/response 모델 truth를 소유하지 않는다.
